...
sub osbb_monthly_fees {
  ...
  foreach my $tarif (@$tarif_list) {
    $debug_output .= "TP ID: $tarif->{id} Living_space: $tarif->{utility_room} utility_room: $tarif->{utility_room} "
    ...
    foreach my $u (@$user_list) {
      ...
      for(my $area_num=0; $area_num<=$#fees_areas; $area_num++ ) {
        if($tarif->{$fees_areas[$area_num]} == 0 || $u->{$fees_areas[$area_num]} ) {
          next;
        }
        ...
        my $sum = $tarif->{$fees_areas[$area_num]};
